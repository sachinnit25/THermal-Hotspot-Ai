...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/status":
            ...
            res = {
                "status": "online",
                "system": "ThermalGuard AI Satellite Classification Engine (Vercel Serverless)",
                "version": "2.0.0",
                "firms_live_mode": Config.is_firms_live(),
                "endpoints": ["/api/hotspots", "/api/stats", "/api/facilities", "/api/config", "/api/refresh", "/api/alerts", "/api/national-risk"]
            }
